...
export const generate30DayStrategy = async (workspaceId, { maxDays = null } = {}) => {
  try {
    ...
    // Calculate actual days ΓÇö cap by maxDays for free plan, otherwise use user's selected duration
...
export const generateContentForSpecificRow = async (workspaceId, entryId) => {
  ...
  // ≡ƒ¢í∩╕Å Ensure complete isolation between Content and Hashtag regeneration.
...
/**
 * STEP 2.5 ΓÇö BRAND LOGO OVERLAY
...
const applyVisualOverlays = async (imageUrl, logoUrl, headingText, subheadingText, aspectRatio = '1:1') => {
  if (!logoUrl && !headingText && !subheadingText) {
    console.log('    [VisualOverlay] ΓÅ¡∩╕Å  Skipping ΓÇö no text or logo to overlay.');
    return imageUrl;
  }

  console.log('    [VisualOverlay] ≡ƒÅ╖∩╕Å  Applying overlays (Logo + Text)...');
  const overlayStart = Date.now();

  try {
    // 1. Download the generated image as base64
    //    Use a longer timeout (45 s) ΓÇö full-res Gemini images can be several MB.
    ...
    if (!imageData || imageData.byteLength < 100) {
      console.warn(`    [VisualOverlay] ΓÜá∩╕Å  Downloaded image is empty/too small (${imageData?.byteLength ?? 0} bytes) ΓÇö skipping overlay.`);
      return imageUrl;
    }

    const imageBase64 = Buffer.from(imageData).toString('base64');
    // Force a valid image MIME ΓÇö GCS signed URLs often return application/octet-stream
      ...
      try {
        ...
        if (rawContentType.includes('icon') || logoUrl.toLowerCase().endsWith('.ico')) {
          try {
            ...
            console.log(`    [VisualOverlay] Γ£à Successfully converted ICO to ${rawContentType}`);
          } catch (e) {
            console.warn(`    [VisualOverlay] ΓÜá∩╕Å Failed to fetch PNG equivalent for ICO: ${e.message}`);
          }
        }

        if (rawBuffer.byteLength >= 100) {
          // Strategy 1: sharp direct conversion ΓåÆ always outputs PNG (Gemini-supported)
          try {
            ...
            // Strategy 2: sharp with failOn:none ΓÇö handles partially corrupt files
            try {
              ...
              if (GEMINI_SUPPORTED_MIMES.includes(rawContentType)) {
                ...
                // ICO, BMP, SVG, TIFF etc ΓÇö Gemini rejects these, skip logo entirely
                console.warn(`    [VisualOverlay] ΓÜá∩╕Å  Logo format "${rawContentType}" is not supported by Gemini ΓÇö skipping logo, text overlay will still apply.`);
              }
            }
          }
        } else {
          console.warn('    [VisualOverlay] ΓÜá∩╕Å  Logo downloaded but appears empty ΓÇö skipping logo.');
        }
      } catch (e) {
        console.warn(`    [VisualOverlay] ΓÜá∩╕Å  Logo download failed (${e.message}), continuing with text only.`);
      }
    }
    ...
    //    IMPORTANT: use 'global' location ΓÇö same as generateImageFromPrompt uses;
...
* Output the final composited image ONLY. No conversational text.`;
    ...
    while (true) {
      try {
        ...
        if (retryCount < maxRetries && isQuotaError) {
          retryCount++;
          console.warn(`    [VisualOverlay] ΓÜá∩╕Å  Quota hit (429). Retrying ${retryCount}/${maxRetries} after ${retryCount * 4}s...`);
    ...
    if (!resultBase64) {
      console.warn('    [VisualOverlay] ΓÜá∩╕Å  Gemini returned no image ΓÇö falling back to original.');
      return imageUrl;
    }
    ...
    if (!gcsResult?.publicUrl) {
      console.warn('    [VisualOverlay] ΓÜá∩╕Å  GCS upload failed ΓÇö falling back to original.');
      return imageUrl;
    }

    console.log(`    [VisualOverlay] Γ£à Overlays composited in ${Date.now() - overlayStart}ms ΓåÆ ${gcsResult.publicUrl.substring(0, 60)}...`);
    return gcsResult.publicUrl;

  } catch (err) {
    console.error(`    [VisualOverlay] Γ¥î Overlay failed (${err.message}) ΓÇö using original image.`);
    return imageUrl;
  }
};

/**
 * AI ADS AGENT ΓÇö VISUAL POST GENERATION PIPELINE
 * ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
 * Step 1   Γöé GPT-4    ΓåÆ Brand-aware Imagen prompt engineering
 * Step 2   Γöé Vertex AI Imagen 3/4 ΓåÆ High-quality visual render
 * Step 2.5 Γöé Gemini 2.5 Flash ΓåÆ Brand logo overlay (top-left)
 * Step 3   Γöé GCS      ΓåÆ Secure cloud storage
 * Step 4   Γöé MongoDB  ΓåÆ GeneratedAsset + Job update
 * Step 5   Γöé Calendar ΓåÆ Entry status marked "generated"
 */
export const generateVisualPostForEntry = async (workspaceId, entryId, jobId, modelId = 'imagen-3.0-generate-001', postFormat = 'single', aspectRatio = '1:1', carouselCount = 3) => {
  ...
  console.log('\n' + 'ΓòÉ'.repeat(60));
  console.log('≡ƒÄ¿  AI ADS AGENT ΓÇö VISUAL POST PIPELINE STARTED');
  ...
  console.log(`  ≡ƒû╝∩╕Å  Format      : ${postFormat.toUpperCase()}`);
  console.log(`  ΓÅ▒  Started at  : ${new Date().toISOString()}`);
  console.log('ΓöÇ'.repeat(60));

  // ΓöÇΓöÇ LOAD: Brand Profile & Calendar Entry ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  if (!brand || !entry) {
    console.error(`[VisualPost] Γ¥î ABORT ΓÇö Data missing: brand=${!!brand}, entry=${!!entry}`);
  ...
  const normalizeAssetType = (raw) => {
    ...
    // Everything else (image, informative, promotional, educational, awareness, etc.) ΓåÆ image
    return 'image';
  };
  const postType = normalizeAssetType(rawPostType);

  console.log(`    Γ£à Brand        : "${companyName}"`);
  console.log(`    Γ£à Post title   : "${title}"`);
  console.log(`    Γ£à Platform     : ${platform} | Type: ${postType} | Phase: ${phase}`);
  console.log(`    Γ£à Brand colors : ${brandColors}`);
  console.log(`    Γ£à Tone         : ${tone}`);
  console.log(`    ΓÅ▒  Loaded in ${Date.now() - dataLoadStart}ms`);

  // ΓöÇΓöÇ STEP 1: GPT-4 Prompt Engineering ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  if (isCarousel) {
    ...
    if (carouselCount === 2) {
      slideStructure += '- Slide 2: CTA-driven closing ΓÇö inspiring action with brand energy';
    } else if (carouselCount === 3) {
      slideStructure += '- Slide 2: Solution / product in context ΓÇö aspirational lifestyle\n';
      slideStructure += '- Slide 3: CTA-driven closing ΓÇö inspiring action with brand energy';
    } else if (carouselCount === 4) {
      slideStructure += '- Slide 2: Problem visualization ΓÇö what challenge the audience faces\n';
      slideStructure += '- Slide 3: Solution / product in context ΓÇö aspirational lifestyle\n';
      slideStructure += '- Slide 4: CTA-driven closing ΓÇö inspiring action with brand energy';
    } else {
      slideStructure += '- Slide 2: Problem visualization ΓÇö what challenge the audience faces\n';
      slideStructure += '- Slide 3: Solution / product in context ΓÇö aspirational lifestyle\n';
      slideStructure += '- Slide 4: Key benefit or proof point ΓÇö data, result, transformation\n';
      slideStructure += '- Slide 5: CTA-driven closing ΓÇö inspiring action with brand energy';
    }
  }
...
Output ONLY the raw Imagen prompt text, nothing else. No JSON, no explanation.`;
  ...
  if (!imagenPrompt || imagenPrompt.trim().length < 20) {
    console.error('[VisualPost] Γ¥î GPT-4 returned empty/invalid prompt');
    throw new Error('GPT-4 returned an empty image prompt');
  }

  const trimmedPrompt = imagenPrompt.trim();
  console.log(`    Γ£à Prompt received (${trimmedPrompt.length} chars) in ${Date.now() - promptStart}ms`);
  
  let finalImagePrompt = trimmedPrompt;
  let carouselSlides = [];

  if (isCarousel) {
    console.log(`    ≡ƒôæ Carousel mode ΓÇö Parsing ${carouselCount} slide prompts...`);
    ...
    if (slidePrompts.length < carouselCount) {
       console.warn(`    ΓÜá∩╕Å  Only parsed ${slidePrompts.length}/${carouselCount} slides. Attempting line-split fallback.`);
    ...
    console.log(`    Γ£à Successfully parsed ${carouselSlides.length} slides.`);
  }

  // ΓöÇΓöÇ STEP 2: Vertex AI Imagen Generation ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  const SLIDE_FRAMES = [
    ...
    { role: 'CTA',      prefix: '',                   suffix: ' ΓÇö Act Now' },
  ];
  ...
  if (isCarousel && carouselSlides.length > 0) {
    console.log(`    ΓÜí Staggered Rendering ${carouselSlides.length} slides (1.2s apart to manage quota)...`);
...
Rules:
- Each "heading" must be DIFFERENT from the others ΓÇö no repetition
- Keep headings under 8 words ΓÇö punchy, bold, suitable for image overlay
...
[
  ...
  { "heading": "...", "subheading": "..." }
    ...
    try {
      ...
      console.log(`    Γ£à Slide text variations ready (${slideTexts.length} slides):`);
      slideTexts.forEach((s, i) => console.log(`       Slide ${i+1}: "${s.heading}" / "${s.subheading}"`) );
    } catch(e) {
      console.warn(`    ΓÜá∩╕Å Vertex variation call failed (${e.message}) ΓÇö using smart local fallback.`);
    ...
    for (let i = 0; i < carouselSlides.length; i++) {
      ...
      try {
        ...
        if (rawSlideUrl) {
          // ΓöÇΓöÇ STEP 2.5: Apply brand logo and text overlay to each slide ΓöÇΓöÇ
          ...
          console.warn(`       ΓÜá∩╕Å  Slide ${i+1} returned empty URL`);
        }
      } catch (err) {
        console.error(`       Γ¥î Slide ${i+1} failed: ${err.message}`);
      }
    ...
    console.log(`    Γ£à ${generatedSlides.length}/${carouselSlides.length} slides rendered successfully.`);
  } else {
    ...
    // ΓöÇΓöÇ STEP 2.5: Apply visual overlays (logo + text) ΓöÇΓöÇ
    imageUrl = await applyVisualOverlays(rawImageUrl, brand.logoUrl, title, hook, aspectRatio);
  }

  if (!imageUrl && generatedSlides.length === 0) {
    console.error('[VisualPost] Γ¥î Vertex AI returned no image URL');
    throw new Error('Vertex AI Imagen returned no image URL');
  }

  const imagenMs = Date.now() - imagenStart;
  console.log(`    Γ£à Generation cycle complete in ${imagenMs}ms`);
  console.log(`    ≡ƒÅ╖∩╕Å  Logo overlay : ${brand.logoUrl ? 'Applied' : 'Skipped (no logo)'}`);
  console.log(`    ≡ƒôÉ Final images  : ${isCarousel ? generatedSlides.length + ' slides' : '1 single image'}`);

  // ΓöÇΓöÇ STEP 3: Save GeneratedAsset to DB ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  console.log(`    Γ£à GeneratedAsset saved: ${asset._id}`);
  console.log(`    ≡ƒôü Asset name   : ${assetName} | Slides: ${generatedSlides.length}`);
  console.log(`    ΓÅ▒  Saved in ${Date.now() - assetStart}ms`);

  // ΓöÇΓöÇ STEP 4: Mark GenerationJob as Completed ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  console.log(`    Γ£à Job ${jobId} ΓåÆ status: "completed"`);

  // ΓöÇΓöÇ STEP 5: Mark CalendarEntry ΓÇö LEAVE STATUS UNCHANGED FOR ISOLATION ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  ...
  // await entry.save();
  console.log(`    Γ£à Entry ${entryId} status preserved for content generation isolation`);

  // ΓöÇΓöÇ PIPELINE COMPLETE ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
  const totalMs = Date.now() - pipelineStart;
  const totalSec = (totalMs / 1000).toFixed(1);
  console.log('\n' + 'ΓòÉ'.repeat(60));
  console.log('Γ£à  AI ADS AGENT ΓÇö PIPELINE COMPLETE');
  console.log('ΓòÉ'.repeat(60));
  console.log(`  ≡ƒåö Asset ID     : ${asset._id}`);
  console.log(`  ≡ƒñû Model used   : ${selectedModel}`);
  console.log(`  ΓÅ▒  Total time   : ${totalSec}s (${totalMs}ms)`);
  console.log(`  ≡ƒöù Image URL    : ${imageUrl.substring(0, 80)}...`);
  console.log('ΓòÉ'.repeat(60) + '\n');

  logger.info(`[VisualPost] Γ£à Pipeline complete in ${totalSec}s | AssetID=${asset._id} | Model=${selectedModel}`);
  return asset;
};
